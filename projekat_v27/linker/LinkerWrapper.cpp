...
LinkerWrapper::LinkerWrapper(string output_file_name_, vector<string> files_to_be_linked_, bool linkable_output_, map<string, int> mapped_section_address_)
    : output_file_name(output_file_name_), files_to_be_linked(files_to_be_linked_), linkable_output(linkable_output_), mapped_section_address(mapped_section_address_),
      linker_output_helper_file("linker_output_helper_file.txt")
{
}
...
bool LinkerWrapper::collect_data_from_relocatible_files()
{
    ...
    linker_output_helper_file << "Collect data from all files:" << endl;
    for (string filename_to_be_linked : files_to_be_linked)
    {

        linker_output_helper_file << filename_to_be_linked << " ";
        ...
        linker_output_helper_file << number_of_relocations << endl;
        linker_output_helper_file << "Relocation data:" << endl;

        linker_output_helper_file << "Offset\tType\t\tDat/Ins\tSymbol\tSection name" << endl;
        vector<RelocationTable> relocation_table_vector;
        for (int i = 0; i < number_of_relocations; i++)
        {
            RelocationTable rel_data;
            linker_output_helper_file << sizeof(rel_data) << endl;
            ...
            linker_output_helper_file << hex << setfill('0') << setw(4) << (0xffff & rel_data.offset) << "\t" << rel_data.type << "\t" << (rel_data.is_data ? 'd' : 'i') << "\t" << rel_data.symbol_name << "\t" << rel_data.section_name << endl;
            relocation_table_vector.push_back(rel_data);
        }
        linker_output_helper_file << dec;
        ...
        linker_output_helper_file << number_of_symbols << endl;
        linker_output_helper_file << "Symbol table:" << endl;
        linker_output_helper_file << "Value\tType\tSection\t\tName\t\tId" << endl;
        for (int i = 0; i < number_of_symbols; i++)
        {
            ...
            linker_output_helper_file << sym_name << endl;
            ...
            linker_output_helper_file << hex << setfill('0') << setw(4) << (0xffff & sym_table.value) << "\t";
            // extern symbols?
            if (sym_table.is_local == true)
                linker_output_helper_file << "l\t";
            else
            {
                if (sym_table.is_defined == true)
                    linker_output_helper_file << "g\t";
                else
                {
                    if (sym_table.is_extern)
                        linker_output_helper_file << "e\t";
                    else
                        linker_output_helper_file << "u\t"; // undefined
                }
            }
            linker_output_helper_file << sym_table.section << "\t" << sym_table.name << "\t" << hex << setfill('0') << setw(4) << (0xffff & sym_table.id_symbol) << endl;
        ...
        linker_output_helper_file << number_of_sections << endl;
        linker_output_helper_file << "Section table:" << endl;
        linker_output_helper_file << "Id\tName\t\tSize" << endl;
        for (int i = 0; i < number_of_sections; i++)
        {
            ...
            linker_output_helper_file << sec_name << endl;
            ...
            linker_output_helper_file << dec << number_of_chars << hex << endl;
            ...
            linker_output_helper_file << dec << sec_table.data.size() << hex << endl;
            linker_output_helper_file << "Data:" << endl;
            ...
            for (char c : sec_table.data)
            {
                if (counter % 16 == 0)
                {
                    linker_output_helper_file << hex << setfill('0') << setw(4) << (0xffff & counter) << "   ";
                }
                linker_output_helper_file << hex << setfill('0') << setw(2) << (0xff & c) << " ";
                counter++;
                if (counter % 16 == 0)
                {
                    linker_output_helper_file << endl;
                }
            }
            linker_output_helper_file << dec;
            linker_output_helper_file << endl;
            ...
            linker_output_helper_file << dec << number_of_offsets << hex << endl;
            ...
            linker_output_helper_file << dec << sec_table.offsets.size() << endl;
            for (int c : sec_table.offsets)
            {
                linker_output_helper_file << c << " ";
            }
            linker_output_helper_file << endl;
            linker_output_helper_file << sec_table.id_section << "\t" << sec_table.section_name << "\t" << hex << setfill('0') << setw(4) << (0xffff & sec_table.size) << endl;
...
bool LinkerWrapper::intersect_of_two_sections(int left_1, int right_1, int left_2, int right_2)
{
    ...
    linker_output_helper_file << "[" << left_1 << "," << right_1 << ")"
                              << "?"
                              << "[" << left_2 << "," << right_2 << ")"
...
bool LinkerWrapper::move_sections_to_virtual_address()
{
         ...
         it_place != mapped_section_address.end(); it_place++)
    {
        ...
        linker_output_helper_file << it_place->first << ":" << it_place->second << endl;
             ...
             it_filename != section_additional_helper[it_place->first].end(); it_filename++)
        {
            ...
            linker_output_helper_file << "F: " << it_filename->first << " :" << it_filename->second.start_addres_in_aggregate_section << endl;
         ...
         it_outer_output_sections != output_section_table.end(); it_outer_output_sections++)
    {
        ...
        linker_output_helper_file << it_outer_output_sections->first << "[" << left_address << "," << right_address << "]" << endl;
    ...
    linker_output_helper_file << "NEXT ADDRESS: " << next_address_for_nonmentioned_sections << endl;
         ...
         it_output_sections != output_section_table.end(); it_output_sections++)
    {
        ...
        if (it_place == mapped_section_address.end())
        {
            ...
            linker_output_helper_file << "sec: " << it_output_sections->first << endl;
            linker_output_helper_file << "VA_b: " << it_output_sections->second.virtual_memory_address << endl;
            ...
            {
                // this will set partial sections beginning and end in aggregate sections
                linker_output_helper_file << "F: " << it_of_section_per_filename->first << " :" << it_of_section_per_filename->second.start_addres_in_aggregate_section << endl;
    ...
    linker_output_helper_file << "Linker output" << endl
                              << endl;
    linker_output_helper_file << "Helper info:" << endl;
    linker_output_helper_file << hex;
    ...
    {
        linker_output_helper_file << "Section: " << it_section->first << " :{" << endl;
        ...
        {
            linker_output_helper_file << "\t"
                                      << "# " << it_filename->first;
            SectionAdditionalData additional_data = it_filename->second;

            linker_output_helper_file << "\t\t" << additional_data.section_name << "\t" << additional_data.parent_file_name;
            linker_output_helper_file << ":\t" << additional_data.parent_section_size;
            linker_output_helper_file << " [" << additional_data.start_addres_in_aggregate_section << ",";
            linker_output_helper_file << additional_data.start_addres_in_aggregate_section + additional_data.parent_section_size << ")";
...
bool LinkerWrapper::create_aggregate_sections()
{
    ...
    {
        linker_output_helper_file << "File: " << filename << endl;
    ...
    for (string filename : files_to_be_linked)
    {
        ...
        for (map<string, SectionTable>::iterator it = section_table_per_file[filename].begin(); it != section_table_per_file[filename].end(); it++)
        {
            linker_output_helper_file << it->second.id_section << "\t" << it->first << "=" << it->second.section_name << "\t" << hex << setfill('0') << setw(4) << (0xffff & it->second.size) << endl;
...
bool LinkerWrapper::create_aggregate_symbol_table()
{
    ...
    for (string filename : files_to_be_linked)
    {
        ...
        for (map<string, SymbolTable>::iterator it = symbol_table_of_file.begin(); it != symbol_table_of_file.end(); it++)
        {
            ...
            {
                linker_output_helper_file << "Extern( " << filename << ": " << it->second.name << ")" << endl;
            ...
            else
            {
                ...
                linker_output_helper_file << "->" << it->first << "=" << it->second.name << ":" << it->second.is_defined << it->second.is_extern << endl;
                it->second.id_symbol = next_symbol_id++;
                linker_output_helper_file << "Should be added: " << section_additional_helper[it->second.section][filename].start_addres_in_aggregate_section << endl;
         ...
         it != extern_symbols_from_every_file.end(); it++)
    {
        ...
        if (sym != output_symbol_table.end())
        {
            linker_output_helper_file << "HERE: " << sym->first << "!" << endl;
        }
        else
        {
            linker_output_helper_file << "EXTERNAL" << endl;
            if (linkable_output == false)
            {
...
bool LinkerWrapper::create_aggregate_relocations()
{
    ...
    for (string filename : files_to_be_linked)
    {
        ...
        {
            linker_output_helper_file << filename << ":" << rel_data.section_name;
            linker_output_helper_file << "(+" << section_additional_helper[rel_data.section_name][filename].start_addres_in_aggregate_section;
            linker_output_helper_file << "-" << output_section_table[rel_data.section_name].virtual_memory_address << ")"
                                      << "\t#";
            linker_output_helper_file << setfill('0') << setw(4) << (0xffff & rel_data.offset) << "\t" << rel_data.type << "\t" << (rel_data.is_data ? 'd' : 'i') << "\t" << rel_data.symbol_name << "\t" << rel_data.section_name << endl;
...
bool LinkerWrapper::create_aggregate_content_of_sections()
{
    for (map<string, SectionTable>::iterator it = output_section_table.begin(); it != output_section_table.end(); it++)
    {
        ...
        linker_output_helper_file << "Section <" << section_name << "(" << it->second.size << ")>:" << endl;
        if (it->second.size == 0)
        {
        ...
        for (string filename : files_to_be_linked)
        {
            ...
            linker_output_helper_file << "S:" << section_additional_data.section_name << " F:" << section_additional_data.parent_file_name << endl;
            ...
            linker_output_helper_file << section_file_offsets.size() << "?" << section_file_data.size() << endl;
            ...
            for (int i = 0; i < section_file_offsets.size() - 1; i++)
            {
                ...
                //linker_output_helper_file << ":" << current_offset << "->" << next_offset << endl;
                linker_output_helper_file << hex << setfill('0') << setw(4) << (0xffff & (current_offset + section_additional_helper[section_name][filename].start_addres_in_aggregate_section)) << ": ";
...
bool LinkerWrapper::solve_relocations_on_data_linkable()
{
    ...
    for (RelocationTable rel_data : output_relocation_table)
    {
        ...
        linker_output_helper_file << rel_data.offset << "[" << rel_data.type << "," << rel_data.is_data << "]"
        ...
        else
        {
            ...
            linker_output_helper_file << " section + " << additional_value << " ! ";
        }
        ...
        if (rel_data.is_data)
        {
            ...
            linker_output_helper_file << higher_value << " " << lower_value << endl;
            /* *** Be careful here! */
            ...
            linker_output_helper_file << " :" << value << " + " << additional_value;
            value += additional_value;
            linker_output_helper_file << " -> " << value << endl;
...
bool LinkerWrapper::solve_relocations_on_data_hex()
{
    ...
    for (RelocationTable rel_data : output_relocation_table)
    {
        ...
        if (it == output_section_table.end())
        {
            ...
            linker_output_helper_file << " symbol + " << additional_value << " ! ";
        }
        else
        {
            // symbol is the section (because it is local symbol in its asm file)
        ...
        if (rel_data.is_data == true)
        {
            ...
            // This is combination of lower and higher Byte. Lower Byte is only 8b and higher byte is expanded (its sign is expanded to the size of an int)
            int value = (int)((higher_value << 8) + (0xff & lower_value));
            linker_output_helper_file << " :" << value << " + " << additional_value << " - " << pc_rel_offset_data;
...
void LinkerWrapper::print_symbol_table()
{
    for (map<string, map<string, SymbolTable>>::iterator it_pf = symbol_table_per_file.begin(); it_pf != symbol_table_per_file.end(); it_pf++)
    {
        ...
        linker_output_helper_file << "Symbol table:" << it_pf->first << endl;
        linker_output_helper_file << "Value\tType\tSection\t\tName\t\tId" << endl;
        for (map<string, SymbolTable>::iterator it = symbol_table.begin(); it != symbol_table.end(); it++)
        {
            linker_output_helper_file << hex << setfill('0') << setw(4) << (0xffff & it->second.value) << "\t";
            // extern symbols?
            ...
            linker_output_helper_file << it->second.section << "\t" << it->second.name << "\t" << hex << setfill('0') << setw(4) << (0xffff & it->second.id_symbol) << endl;
        }
        linker_output_helper_file << dec;
    }
}
void LinkerWrapper::print_section_table()
{
    for (map<string, map<string, SectionTable>>::iterator it_pf = section_table_per_file.begin(); it_pf != section_table_per_file.end(); it_pf++)
    {
        ...
        linker_output_helper_file << "Section table:" << it_pf->first << endl;
        linker_output_helper_file << "Id\tName\t\tSize" << endl;
        for (map<string, SectionTable>::iterator it = section_table.begin(); it != section_table.end(); it++)
        {
            linker_output_helper_file << it->second.id_section << "\t" << it->second.section_name << "\t" << hex << setfill('0') << setw(4) << (0xffff & it->second.size) << endl;
        }
        linker_output_helper_file << dec;
    }
}
...
void LinkerWrapper::print_section_data()
{
    for (map<string, map<string, SectionTable>>::iterator it_pf = section_table_per_file.begin(); it_pf != section_table_per_file.end(); it_pf++)
    {
        ...
        linker_output_helper_file << "Data:" << it_pf->first << endl
                                  << endl;
        for (map<string, SectionTable>::iterator it = section_table.begin(); it != section_table.end(); it++)
        {
            //        if (it->first == "UNDEFINED")
            //            continue;
            linker_output_helper_file << "Section: " << it->first << endl;
...
void LinkerWrapper::print_relocation_table()
{
    for (map<string, vector<RelocationTable>>::iterator it_pf = relocation_table_per_file.begin(); it_pf != relocation_table_per_file.end(); it_pf++)
    {
        ...
        linker_output_helper_file << "Relocation data:" << it_pf->first << endl
                                  << endl;
        linker_output_helper_file << "Offset\tType\t\tDat/Ins\tSymbol\tSection name" << endl;
        for (RelocationTable rel_data : relocation_table)
        {
...
void LinkerWrapper::fill_output_binary_file()
{
    if (linkable_output)
    {
        ...
        linker_output_helper_file << "Create binary file for linker:" << endl;
        ...
        for (RelocationTable rel_data : output_relocation_table)
        {
            linker_output_helper_file << sizeof(rel_data);
        ...
        for (map<string, SectionTable>::iterator it = output_section_table.begin(); it != output_section_table.end(); it++)
        {
            ...
            linker_output_helper_file << number_of_chars << " - " << it->second.data.size() << " => ";
    ...
    else
    {
        ...
        ofstream binary_output_file(binary_filename_output, ios::out | ios::binary);
        linker_output_helper_file << "Create binary file for emulator:" << endl;
        ...
        for (map<string, SectionTable>::iterator it = output_section_table.begin(); it != output_section_table.end(); it++)
        {
            ...
            linker_output_helper_file << "VA:" << virtual_address_of_data << " ; ";
